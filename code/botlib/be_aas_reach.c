...
int AAS_BestReachableFromJumpPadArea(vec3_t origin, vec3_t mins, vec3_t maxs) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		AAS_ClientMovementHitBBox(&move, -1, areastart, PRESENCE_NORMAL, qfalse, qfalse, velocity, cmdmove, 0, 30, 0.1f, bboxmins, bboxmaxs, bot_visualizejumppads);
...
//		Log_Write("shortest distance between %d and %d is %f\r\n", area1num, area2num, bestdist);
		...
		for (i = 0; i < 3; i++) {
			...
			// movement prediction
			AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue, qfalse, velocity, cmdmove, 3, 30, 0.1f, stopevent, 0, qfalse);
	...
	for (i = 0; i < area2->numfaces; i++) {
		...
		for (n = 0; n < 1/*2*/; n++) {
			...
			if (ret && speed < 300) {
				...
				// movement prediction
				AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qtrue, qfalse, velocity, cmdmove, 30, 30, 0.1f, SE_TOUCHTELEPORTER|SE_TOUCHJUMPPAD|SE_HITGROUNDAREA|SE_HITGROUNDDAMAGE|SE_ENTERLAVA|SE_ENTERSLIME|SE_GAP, area2num, visualize);
